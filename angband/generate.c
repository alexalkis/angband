...
//#endif
//
//int32_t GetSysTime(void) {
//	struct DateStamp dt;
//	DateStamp(&dt);
//	return dt.ds_Minute*60*TICKS_PER_SECOND + dt.ds_Tick;
//}
#include <exec/types.h>
ULONG GetSysTime(void);
#define TIME(x) x fflush(stdout);
#else
#define TIME(X)
#endif
...
void cave_generate(struct chunk **c, struct player *p)
{
	...
	for (tries = 0; tries < 100 && error; tries++) {
		...
		TIME(printf("Time pre-generate: %ld\n",GetSysTime());)
		...
		TIME(printf("Time post-clear: %ld\n",GetSysTime());)
	...
	*c = chunk;
	...
	TIME(printf("Time post-cave_clear: %ld\n",GetSysTime());)
	/* Place dungeon squares to trigger feeling (not in town) */
	if (player->depth) {
		place_feeling(*c);
	TIME(printf("Time post-place_feeling: %ld\n",GetSysTime());)
